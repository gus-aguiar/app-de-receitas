...
export async function getMealsFiltered(search) {
  const response = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?c=${search}`);
  const data = await response.json();
  return data.meals;
}

export async function getDrinksFiltered(search) {
  const response = await fetch(`https://www.thecocktaildb.com/api/json/v1/1/filter.php?c=${search}`);
  const data = await response.json();
  return data.drinks;
}
